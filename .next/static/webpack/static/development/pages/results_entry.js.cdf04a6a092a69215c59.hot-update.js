...
function compare(a, b) {
  ...
  if (pointsA > pointsB) {
    comparison = -1;
  } else if (pointsA < pointsB || pointsB == '') {
    comparison = 1;
  }
      ...
      if (arr[a].points !== '' && arr[a].athlete_id !== arr[b].athlete_id) {
        ...
        if (newRank == true) {
          for (var c = 0; c < arr.length; c++) {
            console.log(arr[c].rank + 'rank');

            if (arr[c].rank != '' && arr[c].rank >= rank) {
              arr[c].rank += 1;
            }
          }
        }
...
//# sourceMappingURL=results_entry.js.cdf04a6a092a69215c59.hot-update.js.map